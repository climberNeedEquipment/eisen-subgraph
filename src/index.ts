...
import { Adapter, FetchResultFees } from "./types";
...
const chain = "xdai";
const topic0 =
  "0x7165e2ebc7ce35cc98cb7666f9945b3617f3f36326b76d18937ba5fecf18739a"; //TicketRedeemed
const topic1 =
  "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"; //Transfer
const topic2 =
  "0x000000000000000000000000693bac5ce61c720ddc68533991ceb41199d8f8ae";
...
const methodology = {
  ...
  Revenue:
    "Sum of number of all tickets redeemed multiplied by ticket price in wxHOPR internally in the channels contract and also to the HOPR safe address",
};
...
const fetch = async (timestamp: number): Promise<FetchResultFees> => {
  ...
  const dailyRevenueArrayPaidToSafe = ticketRedeemedLogs
    .map((ticket) => {
      const transactionHash = ticket.transactionHash;
      const index = erc20transferLog.findIndex(
  ...
  const dailyRevenueStayedInChannels = await Promise.all(
    dailyRevenueStayedInChannelsTXs.map(async (transactionHash) => {
  ...
  const dailyRevenue = [
    ...dailyRevenueArrayPaidToSafe,
    ...dailyRevenueStayedInChannels,
  ]
    .map((data: string) => {
